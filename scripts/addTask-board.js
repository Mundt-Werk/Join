 ...
 * The function removes the visual activation of the overlay and hides it after a short delay.
 * @returns {void}
 */
...
function checkInputTitlel(){
  ...
  if (inputTitle) {
    document.getElementById("checkTaskButtonBoard").classList.remove("disable");
    document.getElementById("task_title_error").classList.add("d-none");
    checkTaskButtonBoard.disabled = false;
    border.style.border = ""; 
  }
}
...
function checkValidationInBoard(event){
  
  let hasErrors = validateRequiredFields();
  if (!inputTitle || !inputDate || !inputCategory) {
    checkAddTaskButtonInBoard(hasErrors);
  } else {
    createTaskInBoard(event, hasErrors);
  }
}

function checkAddTaskButtonInBoard(hasErrors){
  if (hasErrors = true) {
    let checkTaskButtonBoard = document.getElementById("checkTaskButtonBoard");
    checkTaskButtonBoard.classList.add("disable");
    checkTaskButtonBoard.disabled = true;
  } else {
    let checkTaskButtonBoard = document.getElementById("checkTaskButtonBoard");
    checkTaskButtonBoard.classList.remove("disable");
    checkTaskButtonBoard.disabled = false;
  }
}
...
async function createTaskInBoard(event, hasErrors) {
  ...
  if (!hasErrors) { 
    ...
    let category = document.getElementById("selected_category_desktop").textContent.trim();
    let priority = document.querySelector(".priority-button.active").textContent.trim();
    let subtasks = newSubtask;
...
async function uploadTasksToAPI(title, description, dueDate, category, priority, subtasks, status, assignedto) {
  let task = {
    titel: title,
